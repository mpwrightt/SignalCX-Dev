-- =============================================================================
-- Row Level Security (RLS) Policies for SignalCX
-- =============================================================================
...
CREATE OR REPLACE FUNCTION auth.has_role_or_higher(required_role user_role)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SECURITY DEFINER
AS $$
    SELECT CASE
        WHEN auth.get_user_role() = 'super_admin' THEN true
        WHEN auth.get_user_role() = 'org_admin' AND required_role != 'super_admin' THEN true
        WHEN auth.get_user_role() = 'manager' AND required_role IN ('readonly', 'agent', 'manager') THEN true
        WHEN auth.get_user_role() = 'agent' AND required_role IN ('readonly', 'agent') THEN true
        WHEN auth.get_user_role() = 'readonly' AND required_role = 'readonly' THEN true
        ELSE false
    END;
$$;
...
CREATE POLICY "Org admins can update their organization"
    ...
    USING (
        auth.has_role_or_higher('org_admin'::user_role) AND
        auth.get_user_organization_id() = id
    );
...
CREATE POLICY "Managers can manage organization users"
    ...
    USING (
        auth.is_user_active() AND
        auth.has_role_or_higher('manager'::user_role) AND
        organization_id = auth.get_user_organization_id()
    )
    WITH CHECK (
        auth.is_user_active() AND
        ...
        (
            auth.get_user_role() = 'super_admin'::user_role OR
            (auth.get_user_role() = 'org_admin'::user_role AND role != 'super_admin'::user_role) OR
            (auth.get_user_role() = 'manager'::user_role AND role IN ('readonly'::user_role, 'agent'::user_role))
        )
    );
...
CREATE POLICY "Agents can manage tickets"
    ...
    WITH CHECK (
        auth.is_user_active() AND
        auth.has_role_or_higher('agent'::user_role) AND
        organization_id = auth.get_user_organization_id()
    );
